...
#line 1 "/home/mzhang/mantaflow/source/gui/mainwindow.cpp"
/******************************************************************************
 *
...
MainWnd::MainWnd() : mPaused(false), mRequestPause(false), mRequestClose(false), mStep(0), QMainWindow(0)
{
    ...
    mGlWidget->set_to_debug(to_debug);
    ...
    for (int i=0; i<(int)mPainter.size(); i++) {
        if(to_debug)
        cout<<"sheldon's comment in MainWnd "<<(mPainter.size())<<endl;
...
bool MainWnd::event(QEvent* e) {
    if (e->type() == (QEvent::Type)EventGuiShow) {
        ...
        if (!mRequestClose) {
            ...
            {
                if(to_debug)
                cerr<<"mRequestPause is ture"<<endl;
                emit painterEvent(Painter::UpdateFull);
            }
            else
            {
                if(to_debug)
                cerr<<"mRequestPause is false"<<endl;
