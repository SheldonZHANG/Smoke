...
struct KnFilamentAdvectParts : public ParticleKernelBase {  KnFilamentAdvectParts (vector<BasicParticleData >& _nodes, vector<BasicParticleData >& _fp, const vector<VortexRing >& _rings, Real _reg, Real _cutoff, Real _scale) : ParticleKernelBase((_nodes).size()), m_nodes(_nodes), m_fp(_fp), m_rings(_rings), m_reg(_reg), m_cutoff(_cutoff), m_scale(_scale), u(size) { run(); } KnFilamentAdvectParts (const KnFilamentAdvectParts& o) : ParticleKernelBase(o.size), m_nodes(o.m_nodes), m_fp(o.m_fp), m_rings(o.m_rings), m_reg(o.m_reg), m_cutoff(o.m_cutoff), m_scale(o.m_scale), u(size) {} inline void op(int i,vector<BasicParticleData >& nodes, vector<BasicParticleData >& fp, const vector<VortexRing >& rings, Real reg, Real cutoff, Real scale, vector<Vec3 >& u)  {
...
} void run() { const int _sz = size; for (int i=0; i < _sz; i++) op(i, m_nodes, m_fp, m_rings, m_reg, m_cutoff, m_scale, u); } operator vector<Vec3>  () { return u; } const vector<Vec3 >& getRet() const { return u; } vector<BasicParticleData >& getArg0() { return m_nodes; } typedef vector<BasicParticleData > type0; vector<BasicParticleData >& getArg1() { return m_fp; } typedef vector<BasicParticleData > type1; const vector<VortexRing >& getArg2() { return m_rings; } typedef vector<VortexRing > type2; vector<BasicParticleData >& m_nodes; vector<BasicParticleData >& m_fp; const vector<VortexRing >& m_rings; Real m_reg; Real m_cutoff; Real m_scale; vector<Vec3 > u;  }; 
...
struct Direct_KnFilamentAdvectParts : public ParticleKernelBase {  Direct_KnFilamentAdvectParts (vector<BasicParticleData >& _nodes, vector<BasicParticleData >& _fp, const vector<VortexRing >& _rings, Real _reg, Real _cutoff, Real _scale) : ParticleKernelBase((_nodes).size()), m_nodes(_nodes), m_fp(_fp), m_rings(_rings), m_reg(_reg), m_cutoff(_cutoff), m_scale(_scale), u(size) { run(); } Direct_KnFilamentAdvectParts (const Direct_KnFilamentAdvectParts& o) : ParticleKernelBase(o.size), m_nodes(o.m_nodes), m_fp(o.m_fp), m_rings(o.m_rings), m_reg(o.m_reg), m_cutoff(o.m_cutoff), m_scale(o.m_scale), u(size) {} inline void op(int i,vector<BasicParticleData >& nodes, vector<BasicParticleData >& fp, const vector<VortexRing >& rings, Real reg, Real cutoff, Real scale, vector<Vec3 >& u)  {
    //cout<<"Enter KnFilamentAdvectParts i="<<i<<endl;
    if (nodes[i].flag & ParticleBase::PDELETE)
        u[i] = _0;
    else
...
void VortexFilamentSystem::divide_ring(Real edge_length,Real dist_threshold){
    ...
    for(int i=0;i<seg_size;i++)
    {
        ...
        for(int j=0;j<oldLen;j++)
        {
            ...
            if(dirty_edge[i].find(r.idx0(j))==dirty_edge[i].end()&&dist>dist_threshold)
            {
                const Vec3 dif=p1-p0;
                int num=floor(dist/edge_length);
                for(int k=1;k<num;k++)
                {
...
void VortexFilamentSystem::merge_adj_edge(Real cosine_threshold){
    ...
    for(int sub=0;sub<seg_size;sub++)
    {
        ...
        for(int i=0;i<oldLen;i++){
            ...
            const Vec3 p2=mData[r.idx1(i+1)].pos;
            ...
            else if(cosine_res>cosine_threshold){
                mData[r.idx1(i)].flag|=PDELETE; 
                mark[(i+1)%oldLen]=true;
                newLen--;
                dirty_edge[sub].insert(i);
