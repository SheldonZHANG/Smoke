...
#line 1 "/home/mzhang/mantaflow/source/gui/painter.cpp"
/******************************************************************************
 *
...
void LockedObjPainter::doEvent(int e, int param) {
    ...
    if (e == UpdateFull) {
        // always update
        if(to_debug)
        cerr<<"LockedObjPainter::doEvent updateFull"<<endl;
        if (mObject) {
            mObject->lock();
            update();
            mObject->unlock();
        ...
        if(to_debug)
        cerr<<"LockedObjPainter::doEvent updateStep"<<endl;
...
void LockedObjPainter::nextObject() {
        if(to_debug)
    cerr<<"Enter LockedObjPainter::nextObject()"<<endl;
    ...
    for(;;) {
        ...
        if(to_debug)
            cerr<<"exit LockedObjPainter::nextObject()"<<endl;
            return;
        }
...
template<> void GridPainter<int>::updateText() {
    stringstream s;
    s << "GridPainter<int> Display Plane " << mPlane << " [" << (char)('X' + mDim) << "]" << endl << endl;
    
    if (mObject) {
        s << "GridPainter<int> Solver '" << mLocalGrid->getParent()->getName() << "'" << endl;
...
template<> void GridPainter<Real>::updateText() {
    stringstream s;
    if (mObject && !mHide) {
        s << "GridPainter<Real>: Real Grid '" << mLocalGrid->getName() << "'" << endl;
...
template<> void GridPainter<Vec3>::updateText() {
    ...
    if (mObject && !mHide && !mHideLocal) {
        s << "GridPainter<Vec3>  Vec Grid '" << mLocalGrid->getName() << "'" << endl;
