...
void PbWrapperRegistry::addClass(const string& pythonName, const string& internalName, const string& baseclass) {
    // store class info
    //cout<<"Commented by sheldon PbWrapperRegistry::addClass("<<pythonName<<", "<<internalName<<","<<baseclass<<endl;
...
PbClassData* PbWrapperRegistry::lookup(const string& name) {
    //cout<<"sheldon's comment PbWrapperRegistry::lookup("<<name<<endl;
    if (mClasses.find(name) != mClasses.end())
        return mClasses[name]; //take notice that the return value type is PbClassData
...
PyObject* cbNew(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    //cout<<"sheldon's comments cbNew("<<type->tp_name<<endl;
    PbObject *self = (PbObject*) type->tp_alloc(type, 0); //PyTypeObject is the python built-in type.. also the same
    //with its function tp_alloc
    if (self != NULL) {
...
PyObject* PbWrapperRegistry::createPyObject(const string& classname, const string& name, PbArgs& args, PbClass *parent) {
    //cout<<"sheldon's comment PbWrapperRegistry::createPyObject("<<classname<<","<<name<<endl;
    PbClassData* classdef = lookup(classname);
    //FlagGrid,MACGrid,RealGrid,Mesh,VortexFilamentSystem,TracerParticleSystem
    if (!classdef)
...
int PbClass::getNumInstances() {
    //cout<<"Enter PbClass::getNumInstances()"<<endl;
    //for(int i=0;i<mInstances.size();i++)
    //    cout<<mInstances[i]->getName()<<endl;
    //cout<<"Exit PbClass::getNumInstances()"<<endl;
...
PbClass* PbClass::createPyObject(const string& classname, const string& name, PbArgs& args, PbClass* parent) {
    //cout<<"sheldon's comment in PbClass::createPyObject("<<classname<<","<<name<<endl;
