...
#line 1 "/home/mzhang/mantaflow/source/fastmarch.cpp"
/******************************************************************************
 *
...
struct SetLevelsetBoundaries : public KernelBase {  SetLevelsetBoundaries (LevelsetGrid& _phi) : KernelBase(&_phi, 0), parent((_phi).getParent()), m_phi(_phi) { run(); } SetLevelsetBoundaries (const SetLevelsetBoundaries& o) : KernelBase(o.maxX, o.maxY, o.maxZ, o.maxCells, o.minZ, o.X, o.Y, o.Z), parent(o.parent), m_phi(o.m_phi) {} inline void op(int i,int j,int k,LevelsetGrid& phi)  {
...
} void run() { const int _maxX = maxX, _maxY=maxY, _maxZ = maxZ; for (int k=minZ; k < _maxZ; k++) for (int j=0; j < _maxY; j++) for (int i=0; i < _maxX; i++) op(i,j,k, m_phi); } FluidSolver* parent; LevelsetGrid& m_phi;  }; 
