...
#line 1 "/home/mzhang/mantaflow/source/mesh.cpp"
/******************************************************************************
 *
...
Real Mesh::computeCenterOfMass(Vec3& cm) const {
    ...
    for(size_t tri=0; tri < mTris.size(); tri++) {
        ...
        double cvol = dot(cross(p1,p2),p3) / 6.0; //this method use the negative/positive of cross product to calculate
        //the volume, so it must ensure that the order of nodes are all clockwise or counter-clockwise
...
void Mesh::rebuildCorners(int from, int to) {
    ...
    for (int c=from*3; c<maxc; c++) {  
        ...
        //sheldon's comment: there are something to note that
        //for a specific node, there maybe several potential opposite nodes,
        //however here we select the one firstly appearing up.
...
struct KnAdvectMeshInGrid : public ParticleKernelBase {  KnAdvectMeshInGrid (vector<Node >& _nodes, const FlagGrid& _flags, const MACGrid& _vel, const Real _dt) : ParticleKernelBase((_nodes).size()), parent((_flags).getParent()), m_nodes(_nodes), m_flags(_flags), m_vel(_vel), m_dt(_dt), u(size) { run(); } KnAdvectMeshInGrid (const KnAdvectMeshInGrid& o) : ParticleKernelBase(o.size), parent(o.parent), m_nodes(o.m_nodes), m_flags(o.m_flags), m_vel(o.m_vel), m_dt(o.m_dt), u(size) {} inline void op(int i,vector<Node >& nodes, const FlagGrid& flags, const MACGrid& vel, const Real dt, vector<Vec3 >& u)  {
...
} void run() { const int _sz = size; for (int i=0; i < _sz; i++) op(i, m_nodes, m_flags, m_vel, m_dt, u); } operator vector<Vec3>  () { return u; } const vector<Vec3 >& getRet() const { return u; } vector<Node >& getArg0() { return m_nodes; } typedef vector<Node > type0; const FlagGrid& getArg1() { return m_flags; } typedef FlagGrid type1; const MACGrid& getArg2() { return m_vel; } typedef MACGrid type2; FluidSolver* parent; vector<Node >& m_nodes; const FlagGrid& m_flags; const MACGrid& m_vel; const Real m_dt; vector<Vec3 > u;  }; 
