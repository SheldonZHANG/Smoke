...
#line 1 "/home/mzhang/mantaflow/source/gui/qtmain.cpp"
/******************************************************************************
 *
...
void guiMain(int argc, char* argv[]) {
    cout<<"commented by sheldon:Entering guiMain"<<endl;
    ...
    {
        cout<<"argv: "<<string(argv[i])<<endl;
        args.push_back(argv[i]);
    }
    

    cout<<"commented by sheldon: args.size()="<<args.size()<<endl;
...
void updateQtGui(bool full, int frame, const string& curPlugin) {    
    //cerr<<"sheldon's comment Entering <<updateQtGui>>  "<<full<<"  "<<frame<<"  "<<curPlugin<<endl;
    if (!gGuiThread->getWindow()->isVisible()) return;
    if (gGuiThread->getWindow()->closeRequest()) throw Error("User interrupt");    
    
    if (full && frame >= 0) gGuiThread->getWindow()->setFrame(frame); //to show the frame numer on the GUI interface
    gMainThread->sendAndWait(full ? (int)MainWnd::EventFullUpdate : (int)MainWnd::EventStepUpdate); //when updateQtGui
    //is called from fluidsolver,full=true;
    
    if (gGuiThread->getWindow()->pauseRequest()) { //mrequestpause&&!mrequestclose
        if (!curPlugin.empty()) { 
            cout << "Step: " << curPlugin <<  endl;
        }
        gGuiThread->getWindow()->setPauseStatus(true); //mpaused=true
        while (gGuiThread->getWindow()->pauseRequest()) {            
            gMainThread->threadSleep(10);
        }
        if (gGuiThread->getWindow()->closeRequest()) throw Error("User interrupt"); //return mrequestclost
