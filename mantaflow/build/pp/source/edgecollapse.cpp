...
#line 1 "/home/mzhang/mantaflow/source/edgecollapse.cpp"
/******************************************************************************
 *
                  ...
                  vector<int> &deletedNodes, std::map<int,bool> &taintedTris, int &numCollapses, bool doTubeCutting)
{
    ...
    for(set<int>::iterator it=ring1.begin(); it != ring1.end(); ++it)  //to check other commonly shared vertices
        //the ring of node n does not contains the index of n itself.
    ...
    {
        cout<<"nonmanifold2 "<<nonmanifold<<" "<<nonmanifold<<"  "<<cl<<endl;
        if(nonmanifold2)  //why?????
            return;
        ...
        if(topTet^botTet) {
            ...
            if(my_mark==true)
            {
                cout<<"in Edgecollapse "<<endl;
                if(cc_old[0].node!=cb_old[2].node||cc_old[1].node!=cb_old[0].node||cc_old[2].node!=cb_old[1].node)
                    cout<<"ERROR Occured"<<endl;
                cout<<cc_old[0].node<<"  "<<cc_old[1].node<<"  "<<cc_old[2].node<<endl;
                cout<<cb_old[2].node<<"  "<<cb_old[0].node<<"  "<<cb_old[1].node<<endl;
                
            }
            
            if (cc_old[0].opposite<0) {cout<<"Fatal Error"<<endl;return;}
            ...
            if(botTet)
            {
                Corner tmp=m.corners(cc_old[0].opposite);
                Corner tmp2=m.corners(ca_old[1].opposite);
                cout<<"check opposite "<<tmp.node<<"  "<<ca_old[1].node<<"  "<<cc_old[0].node<<" "<<tmp2.node<<endl;
            }
            ...
            if(cd_old[0].tri!=ca_old[1].tri&&botTet)
                cout<<"Fatal error in cd_old"<<endl;
