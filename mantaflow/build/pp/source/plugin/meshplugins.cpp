...
#line 1 "/home/mzhang/mantaflow/source/plugin/meshplugins.cpp"
/******************************************************************************
 *
...
void smoothMesh(Mesh& mesh,Real strength,int steps = 1,Real minLength=1e-5, FluidSolver* parent = NULL, PbArgs& _args = PbArgs::EMPTY) {
    /*
      what's this strength used for ???
    ...
    */
    ...
    // temp vertices
    const int numCorners = mesh.numTris() * 3; //maybe it stands for how many angles there are in the mesh
    ...
    for (int s = 0; s<steps; s++) {
        ...
        for (int c = 0; c < numCorners; c++) {
            ...
            for(set<int>::iterator it=ring.begin(); it!=ring.end(); it++) {
                ...
                Real len = norm(edge);//sheldon's comment: compute the magnitude or length of the vector
...
}PyObject* _plugin_smoothMesh (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "smoothMesh"); PyObject *_retval = NULL; { ArgLocker _lock; Mesh& mesh = *_args.get< Mesh* > (0,"mesh", &_lock); Real strength = _args.get< Real > (1,"strength", &_lock); int steps = _args.getOpt< int > (2,"steps", 1, &_lock); Real minLength = _args.getOpt< Real > (3,"minLength", 1e-5, &_lock); _retval = getPyNone();smoothMesh(mesh, strength, steps, minLength, parent);_args.check(); } pbFinalizePlugin(parent,"smoothMesh"); return (_retval) ? _retval : getPyNone(); } catch(std::exception& e) { pbSetError("smoothMesh",e.what()); return 0; } } 

//! Subdivide and edgecollapse to guarantee mesh with edgelengths between
//! min/maxLength and an angle below minAngle
void subdivideMesh(Mesh& mesh,Real minAngle,Real minLength,Real maxLength,bool cutTubes = false, FluidSolver* parent = NULL, PbArgs& _args = PbArgs::EMPTY) {
    ...
    for (int t=0; t<mesh.numTris(); t++) {
        ...
        if (k[0] && k[1])
            CollapseEdge(mesh, t, 2, mesh.getEdge(t,0), mesh.getNode(t,0), deletedNodes, taintedTris, edgeKill,
            cutTubes); //getEdge(t,i) return the edge(i+1,i)
        else if (k[1] && k[2])
    ...
    if (maxLength > 0) {        
        ...
        while(!pq.empty() && pq.top().first>maxLength2) {
            ...
            ca_old[1] = mesh.corners(ca_old[0].next); //(which+1)%3
            ca_old[2] = mesh.corners(ca_old[0].prev); //(which+2)%3
...
}PyObject* _plugin_subdivideMesh (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "subdivideMesh"); PyObject *_retval = NULL; { ArgLocker _lock; Mesh& mesh = *_args.get< Mesh* > (0,"mesh", &_lock); Real minAngle = _args.get< Real > (1,"minAngle", &_lock); Real minLength = _args.get< Real > (2,"minLength", &_lock); Real maxLength = _args.get< Real > (3,"maxLength", &_lock); bool cutTubes = _args.getOpt< bool > (4,"cutTubes", false, &_lock); _retval = getPyNone();subdivideMesh(mesh, minAngle, minLength, maxLength, cutTubes, parent);_args.check(); } pbFinalizePlugin(parent,"subdivideMesh"); return (_retval) ? _retval : getPyNone(); } catch(std::exception& e) { pbSetError("subdivideMesh",e.what()); return 0; } } 
    
void killSmallComponents(Mesh& mesh,int elements = 10, FluidSolver* parent = NULL, PbArgs& _args = PbArgs::EMPTY) {
    cout<<"Enter killSmallComponents"<<endl;
    ...
    cout<<"Exit killSmallComponents"<<endl;
}PyObject* _plugin_killSmallComponents (PyObject* _self, PyObject* _linargs, PyObject* _kwds) { try { PbArgs _args(_linargs, _kwds); FluidSolver *parent = _args.obtainParent(); pbPreparePlugin(parent, "killSmallComponents"); PyObject *_retval = NULL; { ArgLocker _lock; Mesh& mesh = *_args.get< Mesh* > (0,"mesh", &_lock); int elements = _args.getOpt< int > (1,"elements", 10, &_lock); _retval = getPyNone();killSmallComponents(mesh, elements, parent);_args.check(); } pbFinalizePlugin(parent,"killSmallComponents"); return (_retval) ? _retval : getPyNone(); } catch(std::exception& e) { pbSetError("killSmallComponents",e.what()); return 0; } } 
