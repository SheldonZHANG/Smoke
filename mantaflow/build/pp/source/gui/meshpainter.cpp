...
#line 1 "/home/mzhang/mantaflow/source/gui/meshpainter.cpp"
/******************************************************************************
 *
...
MeshPainter::MeshPainter(QWidget* par) 
    : LockedObjPainter(par), mHide(false), mLocalMesh(0) , mBackground(0), 
    mBackgroundMode(BModeNormal), mMode(ModePoints), mVorticityMode(VModeSmoke), mColorScale(1.0)
{    
    mInfo = new QLabel();
}
...
void MeshPainter::updateText() {
    ...
    if (mObject && !mHide) {
        s << "MeshPainter Mesh '" << mLocalMesh->getName() << "' [" << mLocalMesh->numTris() << " tris]" << endl;
