...
#line 1 "/home/mzhang/mantaflow/source/fluidsolver.cpp"
/******************************************************************************
 *
...
FluidSolver::FluidSolver(Vec3i gridsize, int dim,int is_debug)
    //:mDt(1.0), mScale(1.0), mFrame(0), mGridSize(gridsize), mDim(dim), mTimeTotal(0.)
    : PbClass(this), mDt(1.0), mScale(1.0), mFrame(0), mGridSize(gridsize), mDim(dim), mTimeTotal(0.)
{    
    to_debug=is_debug;
    cerr<<"sheldon's comments FluidSolver::FluidSolver"<<gridsize.x<<" "<<gridsize.y<<" "<<gridsize.z<<endl;
...
PbClass* FluidSolver::create(PbType t, const string& name) {        
    ...
    if(to_debug)
    cerr<<"This is sheldon's comment in FluidSolver::create("<<t.str<<" ,"<<name<<")"<<endl;
    if (t.str == "")
        errMsg("Need to specify object type. Use e.g. Solver.create(FlagGrid, ...) or Solver.create(type=FlagGrid, ...)");
    
    PbClass* ret=PbClass::createPyObject(t.str, name, _args, this);
    //cout<<"Sheldon's comment FluidSolver::create "<<(ret->getData())size()<<endl;
    return ret;
}

void FluidSolver::step() {
    if(to_debug)
    cout<<"sheldon's comment FluidSolver::step()"<<endl;
    mTimeTotal += mDt;
    mFrame++;
    ...
    for(size_t i=0;i<mTimings.size(); i++) {
        //cout<<"mTimings.size()="<<mTimings.size()<<endl;
